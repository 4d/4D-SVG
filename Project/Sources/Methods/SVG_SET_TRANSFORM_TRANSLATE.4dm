//%attributes = {"invisible":true,"shared":true,"preemptive":"capable"}
  // ----------------------------------------------------
  // Method : SVG_SET_TRANSFORM_TRANSLATE
...
C_TEXT:C284($kTxt_currentMethod;$Txt_objectID;$Txt_value;$Txt_x)
...
If ($Lon_parameters>=2)
	...
	If (Asserted:C1132(xml_referenceValid ($Txt_objectID);Get localized string:C991("error_badReference")))
		...
		If (OK=1)
			...
			$Lon_start:=Position:C15("translate(";$Txt_value)
		...
		If ($Lon_parameters>=3)
			...
			$Txt_value:=$Txt_value+"translate("+$Txt_x+","+String:C10($3;"&xml")+")"
			
		Else 
			
			$Txt_value:=$Txt_value+"translate("+$Txt_x+",0)"

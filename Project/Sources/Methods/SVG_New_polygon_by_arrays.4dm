//%attributes = {"invisible":true,"shared":true,"preemptive":"capable"}
  // ----------------------------------------------------
  // Method : SVG_New_polygon_by_arrays
  // Created 07/07/08 by Vincent de Lachaux
  // ----------------------------------------------------
  // Description
  // Draw a polygon with the arrays of the coordinates x and y past as arguments
  // ----------------------------------------------------
C_TEXT:C284($0)
C_TEXT:C284($1)
C_POINTER:C301($2)
...
If ($Lon_parameters>=3)
	...
	If (Asserted:C1132(xml_referenceValid ($Txt_parentReference);Get localized string:C991("error_badReference")))
		...
		For ($Lon_i;1;$Lon_Size;1)
			...
			$Txt_Path:=$Txt_Path+String:C10($Ptr_xArray->{$Lon_i};"&xml")+","+String:C10($Ptr_yArray->{$Lon_i};"&xml")+(" "*Num:C11($Lon_i<$Lon_Size))
